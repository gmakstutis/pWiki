---
publish: true
title: The Power of Feedback
modified: 2026-08-08
tags:
  - feedback
  - assessment
  - learning
  - self-regulation
  - meta-analysis
---

# The Power of Feedback

**Author:** John Hattie & Helen Timperley
**Year:** 2007
**Source type:** article
**Raw file:** [Hattie\_Timperly\_2007.pdf](../../raw/articles/Hattie_Timperly_2007.pdf)

## Summary

This article provides a conceptual analysis of feedback and reviews the evidence related to its impact on learning and achievement. The evidence shows that although feedback is among the major influences on learning, the type of feedback and the way it is given can be differentially effective. The article proposes a model of feedback that identifies the particular properties and circumstances that make it effective, and discusses thorny issues including the timing of feedback and the effects of positive and negative feedback. The model identifies three major feedback questions (Where am I going? How am I going? Where to next?) and four levels of feedback (task, processing, self-regulation, and self).

## Key Claims

- Feedback is one of the most powerful influences on learning and achievement, but this impact can be either positive or negative (p. 81).
- Feedback is conceptualized as information provided by an agent (e.g., teacher, peer, book, parent, self, experience) regarding aspects of one's performance or understanding (p. 81).
- Feedback has no effect in a vacuum; to be powerful in its effect, there must be a learning context to which feedback is addressed (p. 82).
- The average effect size of feedback across 12 meta-analyses was 0.79, twice the average effect of schooling (0.40) (p. 83).
- The main purpose of feedback is to reduce discrepancies between current understandings and performance and a goal (p. 86).
- Effective feedback must answer three major questions: Where am I going? (What are the goals?), How am I going? (What progress is being made toward the goal?), and Where to next? (What activities need to be undertaken to make better progress?) (p. 86).
- The model discriminates between four levels of feedback: the task, the processing, the regulatory, and the self levels (p. 102).
- Feedback at the task level is most common and is often called corrective feedback or knowledge of results (p. 91).
- Feedback at the process level appears to be more effective than at the task level for enhancing deeper learning (p. 93).
- Feedback at the self or personal level (usually praise) is rarely effective (p. 102).
- Praise is rarely directed at addressing the three feedback questions and so is ineffective in enhancing learning (p. 102).
- Providing written comments (specific feedback) is more effective than providing grades (p. 92).
- Feedback is most powerful when it addresses faulty interpretations, not a total lack of understanding (p. 82).
- Feedback is not 'the answer'; rather, it is but one powerful answer. With inefficient learners, it is better for a teacher to provide elaborations through instruction than to provide feedback on poorly understood concepts (p. 103).

## Connections

**Concepts:** [Feedback](../concepts/feedback.md) · [Feedback Model](../concepts/feedback-model.md) · [Self-Regulated Learning](../concepts/self-regulated-learning.md) · [Formative Assessment](../concepts/formative-assessment.md) · [Deep Learning](../concepts/deep-learning.md)
**Authors:** [John Hattie](../authors/hattie.md) · [Helen Timperley](../authors/timperley.md)
**Debates:** [Formative vs Summative Assessment](../debates/formative-vs-summative-assessment.md)
**Related source notes:** [Deeley et al 2019](../source-notes/deeley-2019-wicked-problem-assessment-feedback.md) · [Nusche 2008](../source-notes/nusche-2008-assessment-learning-outcomes.md)

## Direct Quotes

> "Feedback is one of the most powerful influences on learning and achievement, but this impact can be either positive or negative." (p. 81)

> "The main purpose of feedback is to reduce discrepancies between current understandings and performance and a goal." (p. 86)

> "Feedback at the self or personal level (usually praise), on the other hand, is rarely effective." (p. 102)

## Open Questions

- How can feedback be targeted at the appropriate level to enhance learning?
- How can teachers and students both seek and learn from feedback?

## Tags

`feedback` `assessment` `learning` `self-regulation` `meta-analysis`
